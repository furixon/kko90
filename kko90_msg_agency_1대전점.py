...
# 지점 선택
try:
    agency = Agency.objects.get(agency_name='대전점')
except Exception as e:
    print('### 해당 지점 정보가 없습니다.', e)
    exit()

# job(agency)
schedule.every().monday.at('17:00').do(job, agency)
schedule.every().tuesday.at('17:00').do(job, agency)
schedule.every().wednesday.at('17:00').do(job, agency)
schedule.every().thursday.at('17:00').do(job, agency)
schedule.every().friday.at('17:00').do(job, agency)
schedule.every().saturday.at('13:30').do(job, agency)
