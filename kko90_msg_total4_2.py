...
def job(agency_all):
    for agency in agency_all:
        ...
        # 드라이버 로딩
        try:
            with open('./kko90_session_{}_2.txt'.format(agency.agency_name), 'r') as f:
        ...
        today_total_msg_count = KkoMsg.objects.filter(agency_name=agency.agency_name, request_at__icontains=today).count()

        if msg_list.exists():
            print('### {} 지점 메시지 전송 시작'.format(agency.agency_name))
            # msg_list_count = int(today_total_msg_count / 2)
