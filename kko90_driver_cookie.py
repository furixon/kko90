...
def get_driver(agency):
    ...
    with open("{}.pickle".format(agency.agency_name), 'wb') as fw:
        print('### 쿠키 파일 저장')
        pickle.dump(jobDriver.get_cookies(), fw)

    return jobDriver

def job(driverData):
    job_jobDriver = driverData[0]  # 크롬 드라이버
    agencyName = driverData[1].agency_name  # 에이전시

    
    print('### {} 지점 Keep Alive!'.format(agencyName))
    job_jobDriver.refresh()

    url = job_jobDriver.command_executor._url
    session_id = job_jobDriver.session_id
...
schedule.every().hours.do(job, [driver, agency])  # 매시 00분에 리프레쉬
