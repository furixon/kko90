...
def job():
    ...
    for msg in msg_list:
        ...
        print('chat select id', chat_select_id)
        ...
        if num_round > 0:
            ...
            # 미확인 메시지 내역 전송
            if msg.agency_name == '유비플러스':
                kko_url_admin = 'https://center-pf.kakao.com/_pZSNb/chats/4839158588628032'  # 관리자 채팅창 접속
            elif msg.agency_name == '신촌홍대점':
                kko_url_admin = 'https://center-pf.kakao.com/_txoxcys/chats/4823474632634460'  # 관리자 채팅창 접속
            else:
                kko_url_admin = 'https://center-pf.kakao.com/_pZSNb/chats/4839158588628032'
            
            kko_report_header = '### 미확인 메시지'
            kko_report = '고객명: {} / 건수: {} / 확인시간:{}'.format(txt_name, num_round, datetime.now().strftime('%Y-%m-%d %H:%M:%S'))

            driver.get(kko_url_admin)
            driver.find_element(By.XPATH, '//*[@id="chatWrite"]').send_keys(kko_report_header)
            driver.find_element(By.XPATH, '//*[@id="chatWrite"]').send_keys(Keys.SHIFT + '\n')
            driver.find_element(By.XPATH, '//*[@id="chatWrite"]').send_keys(kko_report)
            driver.find_element(By.XPATH, '//*[@id="chatWrite"]').send_keys(Keys.SHIFT + '\n')
