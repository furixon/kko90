...
from cms.models import KkoMsg, KkoMsgEvent, Agency, MsgTemplate, PublicMsgTemplate

def job(agency_all):
    for agency in agency_all:
        print('### {} 지점 이벤트 메시지 전송 시작'.format(agency.agency_name))
        ...
        msg_list = KkoMsgEvent.objects.filter(agency_name=agency.agency_name, result='요청')
        ...
        if msg_list.exists():
            ...
            for msg in msg_list:
                ...
                try:
                    kko_url = msg.kko_url
                    kko_msg = PublicMsgTemplate.objects.filter(msg_index=msg.msg_index)[0].msg_content
                    ...
                    if kko_msg is None:  # 메시지 내용이 없을 경우 다음 대상자로 패스
                        ...
                        msg.save()
                        continue
                    kko_msg_line = kko_msg.replace('\r', '').split('\n')
                    kko_image = os.getcwd() + '/media/' + str(PublicMsgTemplate.objects.filter(msg_index=msg.msg_index)[0].img_content)
                    kko_link = PublicMsgTemplate.objects.filter(msg_index=msg.msg_index)[0].link_content
...
target_agency = ['FURIXON', '유비플러스', '울산점']
# target_agency_test = ['FURIXON', '유비플러스']
...
def eventMsgScheduler():
    today = date.today()
    now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    print('### 현재시간 {}'.format(now))
    event_msg_list = PublicMsgTemplate.objects.filter(start_at__date=today).order_by('start_at')
    # event_msg_list = PublicMsgTemplate.objects.all()
    print(event_msg_list)

    for event_msg in event_msg_list:
        print('### schedule => ', event_msg.start_at.strftime('%H:%M:%S'))
        job(agency_all)
        schedule.every().day.at(event_msg.start_at.strftime('%H:%M:%S')).do(job, agency_all).tag('daily_job')


# eventMsgScheduler()
schedule.every().day.at('10:00:00').do(eventMsgScheduler)
